...
export default function Funnel() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("eulogeo_registration");
    if (saved) {
      const data = JSON.parse(saved);
      setName(data.name || ""); setInstitution(data.institution || ""); setPhone(data.phone || ""); setRecordId(data.id || null); setPartnerLink(data.partnerLink || "");
      if (data.id && data.weltradeId) { setWtId(data.weltradeId); setStage(2); } else if (data.id) setStage(1);
    }
  }, []);

  useEffect(() => { if (recordId) localStorage.setItem("eulogeo_registration", JSON.stringify({ name, institution, phone, id: recordId, partnerLink, weltradeId: wtId })); }, [name, institution, phone, recordId, partnerLink, wtId]);
  ...
  async function startRegistration() {
    ...
    try {
      ...
      if (data.existing?.weltrade_id) { setWtId(data.existing.weltrade_id); setStage(2); } else setStage(1);
    } catch (e: any) {
      setError(e.message);
    } finally {
      setBusy(false);
